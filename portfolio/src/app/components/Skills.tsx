    ...
    <section id="skills" className="py-12 bg-gradient-to-b from-slate-900 to-slate-800">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {groups.map((g) => (
            <div key={g.title} className="rounded-lg p-4 bg-slate-800/80 border border-white/10">
              <h3 className="text-lg font-semibold text-slate-100 mb-2">{g.title}</h3>
              <ul className="flex flex-wrap gap-2">
                {g.items.map((i) => (
                  <li key={i} className="text-slate-900 bg-indigo-400 px-2 py-1 rounded text-sm">
                    {i}
                  </li>
                ))}
              </ul>
